...
> **Status:** Building the real self-service Business + Property management foundation — an owner enters/maintains their own production data via the API (no SQL/seeding). **No production Facebook write is activated.** Delivered this sprint (backend + persistence + tests): distinct **Business** vs **Property** entities; additive migration `0012` (`properties`, `property_policies`, `business_contacts`, `business_policies`, `business_audit_events`, `businesses.environment`); structured contacts + policy inheritance; persisted Business + Property readiness (test Business never READY; needs ≥1 active Property); deterministic Property matcher + no-fabrication draft context; workspace-safe CRUD API with audit + 404 cross-workspace; 28 new tests (561 total). **Deferred to Sprint 016** (designed): the mobile-first frontend + onboarding wizard and the full Property-matching PIPELINE integration. Detail: [SPRINT-015](sprints/SPRINT-015-production-business-property-management.md), [100](100-production-business-model.md)–[109](109-property-matching-foundation.md), [ADR-036](adr/ADR-036-business-vs-property-domain.md)/[037](adr/ADR-037-structured-contact-and-policy-model.md)/[038](adr/ADR-038-business-property-readiness.md).
