...
**SPRINT 017 — Owner UX Polish & Guided Onboarding** (branch `feature/s017-owner-ux-polish`, base main `67621c20` / tag `v1.3.0-property-matching`).

> **Status:** Targeted UX sprint to let a non-technical accommodation owner configure the product with minimal assistance — **no new major features, no matching-algorithm change, no Facebook/AI/Telegram/production writes, no new migration**. Delivered: guided 7-step onboarding checklist with progress + next-step CTA on the Business hub (onboarding ≠ Production READY; evaluator stays authoritative); shared Thai property-type labels (enum kept internally); approved-contact surfaced in Human Review (the 7th decision question); plain-language contact-approval labels + helpers; per-option policy guidance with recommended defaults (readiness-required operational fields kept primary); readiness **click-to-fix** (each missing item links to its tab); new owner-facing **ผลการจับคู่** page; property-aware deterministic Mock draft (persisted facts + approved contact, still no fabrication); Thai empty/save states; nav Thai-ification. +20 tests → **617 total, all green**; all quality gates pass. Runtime-validated with synthetic persona คุณเมย์ (Villa B MATCH; draft uses Villa B facts + LINE OA safely); synthetic data cleaned; Pilot job untouched. Detail: [SPRINT-017](sprints/SPRINT-017-owner-ux-polish.md), [125](125-owner-onboarding-ux.md)–[130](130-review-one-screen-decision.md).

### Previously

**SPRINT 016B — Property Match Pipeline, Draft Context & Human Review Integration** (branch `feature/s016b-property-match-pipeline`, base main `2aa89230` / tag `v1.2.0-business-property-self-service`).

> **Status:** Wired persisted Property data into the REAL pipeline — Opportunity → Business Match → **Property Candidates → Property Match → Draft Context → Draft → Human Review**. **No production Facebook write, no external AI, no Telegram, no Collector/Executor.** Delivered: additive migration `0013` (`property_matches` + review snapshot columns; applied, Pilot job `2a85a0dd…` still `succeeded`, 14 migrations); deterministic Property candidate generator (active + same-Business + same-Workspace only) and matcher/ranking (`property-rules-v1`, stable tie-break, NO_PROPERTY_MATCH — no embeddings/AI/fabrication); Property-aware `BusinessContextBuilder` (prompt `rules-v2-property`) with `mustNotClaim` + approved-contacts-only + policy-driven availability/price enforcement; immutable Review snapshot (business/property/property-match ids + context hash) with Thai-first property-match card + warnings; workspace-safe `GET /property-matches[/:id]` + `/property-matching/funnel`; +34 tests → **597 total, all green**; all quality gates pass. Detail: [SPRINT-016B](sprints/SPRINT-016B-property-match-pipeline.md), [118](118-property-match-persistence.md)–[124](124-property-match-observability.md), [ADR-040](adr/ADR-040-property-match-persistence.md)/[041](adr/ADR-041-deterministic-property-selection.md)/[042](adr/ADR-042-review-property-snapshot.md).

### Previously

**SPRINT 016 — Business & Property Frontend (Self-Service UI)** (base main `fe04cdc4` / tag `v1.1.0-business-property-foundation`).

> **Status:** Shipped the mobile-first, Thai-first Business + Property self-service management UI on the Sprint-015 APIs (operator-scoped this turn; the Property-match **pipeline** + Draft-context wiring are **deferred**). **No production Facebook write, no external AI, no Collector, no browser automation.** Delivered: migration `0012` applied (3 businesses preserved as `test`; Pilot job `2a85a0dd…` still `succeeded`); five Next.js pages — business list+filters, create business, tabbed detail hub (ข้อมูลธุรกิจ · ช่องทางติดต่อ · นโยบาย · ที่พัก · ความพร้อมใช้งาน · ประวัติ), create property, tabbed property editor; dependency-free mobile-first design system + Thai label maps; full web API client types/methods; readiness endpoint extended with `environment`+`activePropertyCount`; new `GET /businesses/:id/audit` (auth + ownership + safe projection); `ธุรกิจของฉัน` nav entry; 2 new route tests → **563 total, all green**; `format:check`/`lint`/`typecheck`/`test`/`build`/`doctor` pass. Owners can now complete onboarding + maintenance with **no SQL/seed/CLI**. Detail: [SPRINT-016](sprints/SPRINT-016-business-property-frontend.md), [110](110-business-property-self-service-ui.md), [ADR-039](adr/ADR-039-frontend-self-service-onboarding.md).

### Previously

**SPRINT 015 — Production Business & Property Management** (branch `feature/s015-production-business-property-management`, base `v1.0.0-production-pilot-readiness`).

> **Status:** Built the real self-service Business + Property management foundation — an owner enters/maintains their own production data via the API (no SQL/seeding). **No production Facebook write is activated.** Delivered (backend + persistence + tests): distinct **Business** vs **Property** entities; additive migration `0012` (`properties`, `property_policies`, `business_contacts`, `business_policies`, `business_audit_events`, `businesses.environment`); structured contacts + policy inheritance; persisted Business + Property readiness (test Business never READY; needs ≥1 active Property); deterministic Property matcher + no-fabrication draft context; workspace-safe CRUD API with audit + 404 cross-workspace; 28 new tests. **Frontend shipped in Sprint 016**; the full Property-matching PIPELINE integration remains deferred. Detail: [SPRINT-015](sprints/SPRINT-015-production-business-property-management.md), [100](100-production-business-model.md)–[109](109-property-matching-foundation.md), [ADR-036](adr/ADR-036-business-vs-property-domain.md)/[037](adr/ADR-037-structured-contact-and-policy-model.md)/[038](adr/ADR-038-business-property-readiness.md).
