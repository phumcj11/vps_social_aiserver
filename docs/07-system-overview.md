...
6. **Decide.** The human approves / rejects / edits → the decision reaches the Backend via the Review API → Backend validates and records it (BR-29). A decision **posts nothing** this sprint; a later sprint executes an approved comment. *(SPRINT 010 records; execution later.)*
