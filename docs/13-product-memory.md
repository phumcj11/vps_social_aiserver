...
## Current Stage
SPRINT 006 — Collector Engine (read-only, complete). The first production data pipeline: open groups → navigate → read posts → normalize → store as Signals. No matching, no AI, no Telegram, no writes. See [current-sprint.md](current-sprint.md).

## Terminology (renames, SPRINT 006)
**Scanner → Collector** · **Lead → Opportunity** · **Post → Signal** · **Comment Worker → Action Worker**. A **Signal** is platform-independent (today a Facebook post; tomorrow a TikTok video, Instagram reel, or LINE message). See [ADR-010](adr/ADR-010-signal-model.md).
...
## Current Sprint
SPRINT 006 — Collector Engine (read-only, complete). Modular pipeline that opens groups, reads posts, normalizes, and stores platform-neutral Signals; duplicate detection; checkpoints; runs. No matching, AI, Telegram, comment, opportunity, or writes.

## Next Sprint
SPRINT 007 — Business Matching and AI Draft: match Signals to businesses and generate business-specific drafts, with scores and explanations. See [12-mvp-roadmap.md](12-mvp-roadmap.md).
